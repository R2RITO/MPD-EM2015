...
*/
	...
	if($_FILES['file']['name'] != '')
	{
		...
		for($i=1; $i<$numh ;$i++){
				...
				//$fbd1 -> agregarTonoFlexEtiquetasBD($etiquetas[$i]);
				for($j=1;$j<$numh;$j++){
				
					//echo "++".$etiquetas[$i]."-".$rango[$j]."-".$valores[$i][$j]."<br>";
					$fbd = FachadaBD::getInstance();
					$caract = NEW dominio_fijo_t();
					$caract->setDom ($_POST["dorsal"]);
					$caract->setEt($etiquetas[$i]);
					$caract->setVal($rango[$j]);
					$caract->setGrado($valores[$i][$j]);
					$fbd -> agregarTonoFlexDorBD($caract);
		...
		<div class="row">
			<div class="12u">
				...
				<h2>Tono Muscular</h2>
					...
					<p align="right"><a  href="f_indexE.php" class="button-style1">Continuar</a></p>
				</section>
